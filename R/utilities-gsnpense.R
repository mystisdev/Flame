# =============================================================================
# FLAME gSNPense Service
# =============================================================================
#
# Service function for the gSNPense API (gprofiler2 package).
...
gsnpense_convert <- function(snp_ids, filter_na = TRUE) {
  tryCatch({
    if (length(snp_ids) == 0) {
      cat("[gSNPenseService] No SNP IDs provided\n")
      return(NULL)
    }
    ...
    if (is.null(result) || nrow(result) == 0) {
      cat("[gSNPenseService] No results returned from gsnpense\n")
      return(NULL)
    }

    return(result)
  }, error = function(e) {
    cat(paste("[gSNPenseService] gsnpense_convert error:", conditionMessage(e), "\n"))
    return(NULL)
  })
}
